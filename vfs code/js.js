...
// Login state
let loginIdx=0,loginBusy=false;
const getLogin=()=>{const a=loadLogins();if(loginIdx>=a.length)loginIdx=0;return a[loginIdx];};
const nextLogin=()=>{loginIdx=(loginIdx+1)%loadLogins().length;updateLoginBadge();};
...
new MutationObserver(()=>{
  try{
    ...
    if(wlDone&&document.body.innerText.includes('Summary')){const b=btnTxt('continue');if(b)b.click();}
    if(!svcDone&&document.querySelector('app-manage-service')){const b=btnTxt('continue');if(b){b.click();svcDone=true;}}
    if(!svcDone&&document.body.innerText.includes('Summary')){const b=btnTxt('continue');if(b){b.click();svcDone=true;}}
...
function tryClickGoHome(){
  try{
    ...
    _rt(()=>{try{a.click();}catch(e){}},400+Math.random()*300);
  }catch(e){}
}
